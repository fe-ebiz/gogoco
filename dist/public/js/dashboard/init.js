...
function rstChart(num) {
    ...
    var date = new Date(),
        ...
        dateVal = new Date(year, month+Number(num), 0), 
        dayDate = dateVal.getDate(),
        legendMonth, xNum;
    
    if( String(month+Number(num)).length < 2 ) {
        legendMonth = String("0")+String(month+Number(num));
    }
        
    var legend = year +'-'+legendMonth;

    //console.log(year, month+Number(num), dayDate);
    ...
    for( var i = 0 ; i < dayDate ; i++ ){
        ...
        label[i] = xNum+week[new Date(Number(year)+'-'+Number(month+Number(num))+'-'+Number(xNum)).getDay()];
    }
